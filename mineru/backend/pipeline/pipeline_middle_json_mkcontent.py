...
def make_blocks_to_markdown(paras_of_layout,
    ...
    for para_block in paras_of_layout:
        ...
        if para_type in [BlockType.TEXT, BlockType.LIST, BlockType.INDEX, BlockType.CHEMICAL]:
...
def merge_para_with_text(para_block):
    ...
    for i, line in enumerate(para_block['lines']):
        ...
        for j, span in enumerate(line['spans']):
            ...
            elif span_type == ContentType.CHEMICAL_FORMULA:
                if span.get('content', ''):
                    content = f"<smiles>{span['content']}</smiles>"
            ...
            if content:
                ...
                if block_lang in langs: # 中文/日语/韩文语境下，换行不需要空格分隔,但是如果是行内公式结尾，还是要加空格
                    ...
                    if span_type in [ContentType.TEXT, ContentType.INLINE_EQUATION, ContentType.CHEMICAL_FORMULA]:
...
def make_blocks_to_content_list(para_block, img_buket_path, page_idx, page_size):
    ...
    elif para_type == BlockType.CHEMICAL:
        para_content = {
            'type': BlockType.CHEMICAL,
            'text': merge_para_with_text(para_block),
        }
